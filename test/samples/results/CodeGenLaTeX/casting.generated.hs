...
module TestModule (TestModule.f, TestModule.g, TestModule.h, TestModule.i, TestModule.j, TestModule.k, TestModule.l) where
...
f a b
  | otherwise = (widen @Integer a) - (widen @Integer b)

g :: Integer -> Natural
g x
  | x < (widen @Integer (0 :: Natural)) = 0 :: Natural
  | otherwise = narrow @Natural x

h :: Double
h 
  | otherwise = (1.25 :: Double) - (widen @Double ((widen @Integer (1 :: Positive)) % (widen @Integer (2 :: Positive))))

i :: Positive
i 
  | otherwise = narrow @Positive (sqrt (widen @Double (9 :: Positive)))

j :: Positive
j 
  | otherwise = narrow @Positive ((widen @Integer (12 :: Positive)) % (widen @Integer (3 :: Positive)))

k :: Rational
k 
  | otherwise = narrow @Rational (0.25 :: Double)

l :: Integer
l 
  | otherwise = narrow @Integer ((widen @Double (10 :: Positive)) * ((0.1 :: Double) + (0.2 :: Double)))
