...
module TestModule (TestModule.f, TestModule.g, TestModule.h, TestModule.i, TestModule.j, TestModule.k) where
...
f :: Natural -> Positive
f x
  | otherwise = narrow @Positive (y * (-(widen @Integer (2 :: Positive))))
  where
    y = -(widen @Integer (x + (widen @Natural (1 :: Positive))))

g :: Integer -> Natural -> Rational
g x a
  | otherwise = (x ^ a) % (widen @Integer (2 :: Positive))

h :: Integer -> (Integer, Double)
h x
  | otherwise = (x, widen @Double x)

i :: Integer -> (Integer, Integer)
i x
  | otherwise = ((widen @Integer (2 :: Positive)) * a, (widen @Integer (2 :: Positive)) * b)
  where
    (a, b) = (x, x)

j :: Integer -> (Natural, Integer)
j x
  | x >= (widen @Integer (0 :: Natural)) = (narrow @Natural x, x)
  | otherwise = (narrow @Natural (-x), x)

k :: Integer -> (Natural, Integer)
k x
  | not (x /= (widen @Integer (0 :: Natural))) = error "[k] Violated constraint `IRBinary IRNeq (IRCall "x" False []) (IRCast (IRImmediateInt 0 Natural) Natural Integer)`"
  | not (y /= (widen @Integer (0 :: Natural))) = error "[k] Violated constraint `IRBinary IRNeq (IRCall "y" False []) (IRCast (IRImmediateInt 0 Natural) Natural Integer)`"
  | x > (widen @Integer (0 :: Natural)) = (narrow @Natural x, x)
  | otherwise = (narrow @Natural y, x)
  where
    y = -x
